...
Use `aiwiki codex "<task>"` when Codex needs a full runbook, `aiwiki codex
"<task>" --team` when the human explicitly wants a multi-agent workflow, and
`aiwiki brief "<task>"` when a compact development brief is enough.
...
previews by default. Confirmed writes only create or append supported wiki pages
under `.aiwiki/wiki/`.
...
For larger memory sets, `aiwiki index build` creates a derived hybrid index:
...
files can contribute to review. It can draft concrete memory candidates for
work-graph changes, structured JSON errors, and semantic risk lessons, and it
can suggest append refreshes for wiki pages whose `files` frontmatter references
changed code. These are still candidates: use `aiwiki apply <plan.json>` to
...
work needs coordination. The rest of the command surface is for maintaining
memory, graph relations, module packs, eval fixtures, and reviewed updates.
...
ready unblocked work, memory health, and the next few commands. Use
...
repositories into a cache directory, runs `prime`, `codex --team`, and `guard`,
then fails if the expected language risk signals disappear.

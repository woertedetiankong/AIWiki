...
- For explicit session-captured lessons, use `aiwiki session scan` to inspect matching local agent sessions and `aiwiki session reflect --output-plan <path>` to draft proposed memory. Treat session traces as external context; never let them write long-term wiki memory without `aiwiki apply` preview and user approval.
